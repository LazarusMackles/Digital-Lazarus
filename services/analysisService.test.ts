...
describe('analysisService', () => {
    ...
    describe('buildPrompt', () => {
        ...
        it('should include the reanalysis directive when isReanalysis is true', () => {
            ...
            expect(prompt).toContain('PRIORITY DIRECTIVE: ADVERSARIAL SECOND OPINION');
        });
    });


    // --- finaliseForensicVerdict Tests ---
    describe('finaliseForensicVerdict', () => {
        const rawResult = {
            probability: 95,
        ...
        it('should use Hive score and align verdict when provided', () => {
            const result = finaliseForensicVerdict(rawResult, 30);
        ...
        it('should clamp probability based on verdict when no external score is given', () => {
            const humanResult = finaliseForensicVerdict({ ...rawResult, verdict: 'Appears Human-Crafted', probability: 88 });
            expect(humanResult.probability).toBe(35);
            
            const aiResult = finaliseForensicVerdict({ ...rawResult, verdict: 'Fully AI-Generated', probability: 30 });
