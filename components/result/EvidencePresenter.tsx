                        ...
                        <>
                            ...
                            <div className="mt-4 w-full max-w-2xl grid grid-cols-2 sm:grid-cols-4 gap-4">
                                {files.map((file, index) => (
                                    <div key={`${file.name}-${index}`} className="relative group aspect-square bg-slate-100 dark:bg-slate-900/50 rounded-lg overflow-hidden border border-slate-300 dark:border-slate-700 cursor-pointer" onClick={() => setLightboxImage(file.imageBase64)}>
